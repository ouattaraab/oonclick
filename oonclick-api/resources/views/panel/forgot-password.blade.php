...
<head>
    ...
    <title>Mot de passe oublié — oon.click Panel</title>
...
<body>
    <div class="login-card">
        ...
        <div class="login-heading">
            ...
            <p>Saisissez votre email pour recevoir un code de réinitialisation</p>
        </div>
        ...
        {{-- Form --}}
        @if(! session('status'))
        <form method="POST" action="{{ route('panel.password.email') }}">
            @csrf
            ...
            <button type="submit" class="btn-login">Envoyer le code</button>
        </form>
        @else
        <a href="{{ route('panel.password.reset') }}?email={{ urlencode(session('password_reset_email', '')) }}" class="btn-login" style="display:block;text-align:center;text-decoration:none;padding:13px;">
            Saisir mon code de réinitialisation
        </a>
        @endif

        <a href="{{ route('panel.login') }}" class="back-link">&#8592; Retour à la connexion</a>
